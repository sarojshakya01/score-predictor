...
const LeaderboardRow = ({
  ...
  return (
    <tr className={[
      "transition-colors",
      row.user_id === user?.id ? "bg-zinc-200 dark:bg-sky-950 hover:bg-zinc-200/80 dark:hover:bg-sky-900/80 font-bold" : "hover:bg-zinc-50/70 dark:hover:bg-zinc-800/40 font-medium",
    ].join(" ")}>
      <td className={[
        "static sm:sticky left-0 z-20 w-[30px] min-w-[30px] max-w-[30px]",
        "border-b border-zinc-200 dark:border-zinc-800",
        "px-3 py-4 font-semibold text-zinc-950 dark:text-zinc-100 text-center",
        row.user_id === user?.id ? "bg-zinc-200 dark:bg-sky-900/40 font-bold" : "bg-white dark:bg-zinc-950",
      ].join(" ")}>{row.rank}</td>
      <td className={[
