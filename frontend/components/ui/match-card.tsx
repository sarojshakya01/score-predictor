...
const MatchTitle = (match: MatchResponse) => {
  return <dl className="items-start justify-between gap-3 min-h-[60px] content-center">
    <div className="flex items-center justify-center">
      <h2 className="w-full mt-2 text-md font-semibold text-zinc-950">
  ...
  return (
    <article className="rounded-md border border-zinc-200 bg-white p-4 shadow-sm">
      ...
      <dl className="mt-5 grid grid-cols-2 gap-4 text-sm">
        <div>
          <dt className="text-zinc-500">Kickoff</dt>
          <dd className="mt-1 font-medium text-zinc-950">{formatDateTime(match.match_datetime)}</dd>
        </div>
        <div className="justify-self-end">
          <dt className="text-zinc-500">Group</dt>
          <dd className="mt-1 font-medium text-zinc-950">{formatMatchGroup(match)}</dd>
        </div>
      </dl>
      {MatchVenue(match)}
