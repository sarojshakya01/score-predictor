...
const AdminSettingsPage = () => {
  ...
  const [openingEditSettingId, setOpeningEditSettingId] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (isMounted) {
          showToast({ tone: "error", message: getErrorMessage(err, "Unable to load settings.") });
        }
      } finally {
  ...
  const handleOpenEditModal = async (setting: SettingResponse) => {
    setJsonError(null);
    setFormError(null);
    setOpeningEditSettingId(setting.id);
    try {
      const freshSetting = await getAdminSetting(setting.id);
      setSettings((current) =>
        current.map((s) => (s.id === freshSetting.id ? freshSetting : s)),
      );
      setEditingSettingId(freshSetting.id);
      setFormState({
        name: freshSetting.name,
        friendly_name: freshSetting.friendly_name,
        valueRaw: prettyJson(freshSetting.value),
      });
      setIsModalOpen(true);
    } catch (err) {
      showToast({ tone: "error", message: getErrorMessage(err, "Unable to load the latest setting details.") });
    } finally {
      setOpeningEditSettingId(null);
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      showToast({ tone: "error", message: getErrorMessage(err, "Unable to delete setting.") });
    } finally {
