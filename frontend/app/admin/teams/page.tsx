...
const AdminTeamsPage = () => {
  ...
  const [openingEditTeamId, setOpeningEditTeamId] = useState<number | null>(null);
  ...
  const handleOpenEditModal = async (team: TeamResponse) => {
    setLoadError(null);
    setFormError(null);
    setOpeningEditTeamId(team.id);

    try {
      const freshTeam = await getAdminTeam(team.id);
      setTeams((current) =>
        current.map((currentTeam) =>
          currentTeam.id === freshTeam.id ? freshTeam : currentTeam,
        ),
      );
      setEditingTeamId(freshTeam.id);
      setFormState({
        name: freshTeam.name,
        group: freshTeam.group,
        fifa_code: freshTeam.fifa_code,
        fifa_rank: freshTeam.fifa_rank,
      });
      setIsModalOpen(true);
    } catch (error) {
      setLoadError(getErrorMessage(error, "Unable to load the latest team details."));
    } finally {
      setOpeningEditTeamId(null);
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      showToast({ tone: "error", message: getErrorMessage(error, "Unable to delete team.") });
    } finally {
                    ...
                    <tr key={team.id} className="transition-colors hover:bg-zinc-50/70 dark:hover:bg-zinc-800/40">
                      ...
                      <td className="px-5 py-4 font-medium text-zinc-950 dark:text-zinc-50">
                        <div className="flex items-center gap-2">
                          <Image width={30} height={30} className="min-h-[25px] w-auto rounded object-cover shadow-sm" decoding="async" loading="lazy" src={team.flag_url === "default" ? defaultFlag : team.flag_url} alt="flag" />
                        ...
                        <div className="flex justify-end gap-1">
                          <Tooltip content="Edit">
                            <button
                              ...
                              disabled={openingEditTeamId !== null}
