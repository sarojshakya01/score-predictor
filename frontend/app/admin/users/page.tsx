...
const AdminUsersPage = () => {
  ...
  const [openingEditUserId, setOpeningEditUserId] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    const loadUsers = async () => {
      ...
      try {
        ...
        if (isMounted) {
          showToast({ tone: "error", message: getErrorMessage(error, "Unable to load users.") });
        }
      } finally {
        if (isMounted) {
          setIsLoading(false);
  ...
  const handleOpenEditModal = async (user: UserResponse) => {
    setFormError(null);
    setOpeningEditUserId(user.id);
    ...
    try {
      ...
      showToast({ tone: "error", message: getErrorMessage(error, "Unable to load the latest user details.") });
    } finally {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      showToast({
        tone: "success",
        title: "User deleted",
        message: `${user.first_name} ${user.last_name} has been deleted.`,
      });
    } catch (error) {
      showToast({ tone: "error", message: getErrorMessage(error, "Unable to delete user.") });
    } finally {
                ...
                {isLoading ? (
                  ...
                  pagedUsers.map((user, idx) => (
                    <tr key={user.id} className="transition-colors hover:bg-zinc-50/70 dark:hover:bg-zinc-800/40">
                      <td className="px-5 py-4 text-left text-zinc-700 dark:text-zinc-300">{((page - 1) * PAGE_SIZE) + idx + 1}</td>
          ...
          <form onSubmit={(e) => void handleSubmit(e)} className="flex flex-col gap-4">
            ...
            <div className="mt-4 flex justify-end gap-3">
              <button
                ...
                className="inline-flex h-11 px-4 items-center gap-2 justify-center cursor-pointer rounded-md border border-zinc-200 bg-white text-sm font-semibold text-zinc-700 transition hover:border-zinc-300 hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:border-zinc-600 dark:hover:bg-zinc-700"
              >
              ...
              <button
                ...
                className="inline-flex h-11 px-4 items-center gap-2 justify-center cursor-pointer rounded-md bg-tournament-primary text-sm font-semibold text-white transition hover:bg-tournament-primary disabled:cursor-not-allowed disabled:bg-zinc-400"
              >
